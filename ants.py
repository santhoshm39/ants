...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper_nearest(curr_place, k):
            curr_bee = rANTdom_else_none(curr_place.bees)
            if curr_bee and k >= self.min_range and k <= self.max_range:
                return curr_bee
            elif curr_place.entrance is not beehive:
                return helper_nearest(curr_place.entrance, k + 1)
            else:
                return None
        return helper_nearest(self.place, 0)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        def fire_helper(damage_amount):
            for bee in self.place.bees[:]:
                Insect.reduce_armor(bee, damage_amount)
            Ant.reduce_armor(self, amount)

        if self.armor - amount <= 0:
            fire_helper(amount + self.damage)
        else:
            fire_helper(amount)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        def damage_double(ant):
            if hasattr(ant, "damage") and not ant.buffed:
                ant.damage *= 2
                ant.buffed = True
        def queen_double_power(place):
            if place:
                if place.ant:
                    if isinstance(place.ant, ContainerAnt):
                        damage_double(place.ant)
                        if place.ant.contained_ant:
                            damage_double(place.ant.contained_ant)
                    else:
                        damage_double(place.ant)
                return queen_double_power(place.exit)
        if self.impostor:
            Insect.reduce_armor(self, self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            queen_double_power(self.place.exit)
        # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        #True if self.place.ant.blocks_path else False
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def backwards_action(gamestate):
        bee.forward_direction = False
        action(gamestate)
        bee.forward_direction = True
    return backwards_action
    # END Problem EC
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    count = length
    new, orig = status(bee.action, bee), bee.action
    def action_helper(gamestate):
        nonlocal count, new, orig
        print("DEBUG:", count)
        if count > 0:
            count -= 1
            new(gamestate)
